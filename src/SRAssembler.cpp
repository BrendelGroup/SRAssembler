...
//	usage.append("-1: Required if the -l option is not used; use this option to specify the\n");
//	usage.append("    single-end reads file or the left-end reads file for paired-end reads.\n");
...
//	usage.append("\n");
//	usage.append("-A: Contig assembler program; options: 0=>SOAPdenovo2, 1=>ABySS. [Default: " + int2str(ASSEMBLER_PROGRAM) + "]\n");
	usage.append("-k: Specifies the k-mer set to be used by the assembler. [Default: " + int2str(START_K) + ":" + int2str(STEP_K) + ":" + int2str(END_K) + "]\n");
//	usage.append("    Format: start_k:interval:end_k. Start_k and end_k must be odd integers, and\n");
//	usage.append("    interval must be an even integer, as in the following example:\n");
//	usage.append("    '15:10:45' specifies that k-mer values 15, 25, 35, 45 will be tested.\n");
//	usage.append("-S: Spliced alignment program; 0=>GeneSeqer, 1=>GenomeThreader. [Default: " + int2str(SPLICED_ALIGNMENT_PROGRAM) + "]\n");
//	usage.append("-s: Species model for spliced alignment; options: 'human', 'mouse', 'rat', \n");
//	usage.append("    'chicken', 'drosophila', 'nematode', 'aspergillus', 'fission_yeast', \n");
//	usage.append("    'arabidopsis', 'maize', 'rice', 'medicago'.\n");
//	usage.append("    [DEFAULT: " + DEFAULT_SPECIES + "]\n");
...
//	usage.append("-E: Extra rounds of recursion performed once a hit contig is found. [Default: " + int2str(EXTRA_ROUNDS) + "]\n");
//	usage.append("-a: The round in which to start read assembly. [Default: " + int2str(ASSEMBLY_ROUND) + "]\n");
//	usage.append("-b: The frequency with which to periodically remove unrelated contigs and reads.\n");
//	usage.append("    For example, '-b 3' specifies that SRAssembler will purge after two rounds\n");
//	usage.append("    of not doing so. [Default: " + int2str(CLEAN_ROUND) + "]\n");
//	usage.append("-d: The minimum number of assembled contigs to automatically trigger removal of\n");
//	usage.append("    unrelated contigs and reads. If set to '0', do not remove unrelated contigs\n");
//	usage.append("    and reads except as scheduled by '-b' option. [Default: " + int2str(CONTIG_LIMIT) + "]\n");
//	usage.append("-j: FASTA-formatted file containing sequences used to taboo reads and prevent\n");
//	usage.append("    them from being used for assembly.\n");
...
//	usage.append("-x: The round in which to start masking the center of query contigs when\n");
//	usage.append("    searching for new reads. If set to '0', do not mask center of query contigs.\n");
//	usage.append("    [Default: " + int2str(END_SEARCH_ROUND) + "]\n");
...
//	usage.append("-z: The round in which to start masking low-complexity regions of contigs before\n");
//	usage.append("    searching for reads. If set to '0', do not mask low-complexity regions of\n");
//	usage.append("    query contigs. [Default: " + int2str(MASKING_ROUND) + "]\n");
	usage.append("\n");
//	usage.append("-v: Verbose output (typically only used for debugging).\n");
	usage.append("-h: Print extended usage synopsis.");

	extended_usage = "SRAssembler v" + VERSION + " extended_usage:\n";
	extended_usage.append("    SRAssembler [options] -q query_file -p parameter_file\n");
	extended_usage.append("                [-l library_file OR -1 reads_file1 -2 reads_file2 -Z insertsize]\n");
	...
	extended_usage.append("    interval must be an even integer, as in the following example:\n");
	extended_usage.append("    '15:10:45' specifies that k-mer values 15, 25, 35, 45 will be tested.\n");
	...
	extended_usage.append("    'arabidopsis', 'maize', 'rice', 'medicago'.\n");
	extended_usage.append("    [DEFAULT: " + DEFAULT_SPECIES + "]\n");
